...
`,
    ...
    beforeScreenshot: async page => {
        ...
        await page.$eval("#gvp-report-preview-image", (node, logo) => node.src = logo, logo);

        for (const [tag, text] of tagsDisplayText) {
...
`,
    ...
    beforeScreenshot: async page => {
        await installMouseHelper(page);
        await page.setViewport({ width: 1920, height: 1080 });
        await page.$eval("#gvp-image-preview", (node, logo) => node.src = logo, logo);
...
`,
    ...
    beforeScreenshot: async page => {
        ...
        await page.evaluate((generateFilteredImage) => {
            const width = 960;
            const height = 540;
