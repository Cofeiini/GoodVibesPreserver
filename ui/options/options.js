import {
__toESM,
require_client,
require_react
} from "../chunk-ff9566dbb8bbd2d1.js";

// ui/options/options.tsx
var React = __toESM(require_react(), 1);
var client = __toESM(require_client(), 1);

// assets/options-settings-icon.svg
var options_settings_icon_default = "../options-settings-icon-2e735c8e3ab18031.svg";

// assets/help-icon.svg
var help_icon_default = "../help-icon-6ed7dbe4ee79353b.svg";

// assets/privacy-icon.svg
var privacy_icon_default = "../privacy-icon-3f20322dbc239a51.svg";

// assets/customize-icon.svg
var customize_icon_default = "../customize-icon-518c53d1e52189fe.svg";

// assets/github-icon.svg
var github_icon_default = "../github-icon-5aaa6fb43c3efe1b.svg";

// assets/contact-icon.svg
var contact_icon_default = "../contact-icon-a26f7e2a77a6c2b5.svg";

// ui/options/options.tsx
var PrivacySection = () => {
  return React.createElement("div", null, "privacy;");
};
var CustomizeSection = () => {
  return React.createElement("div", null, "Customize;");
};
var SettingsSection = () => {
  return React.createElement("div", null, "Settings;");
};
var HelpSection = () => {
  return React.createElement("div", null, "Help;");
};
var MenuItem = ({ itemName, itemIcon, isSelected, selectCallback }) => {
  const [hasLabel, setHasLabel] = React.useState(true);
  React.useLayoutEffect(() => {
    const checkSize = () => {
      const menuItemTarget = document.querySelector(".menu-item");
      if (menuItemTarget) {
        setHasLabel(menuItemTarget.offsetWidth > 158);
      }
    };
    window.addEventListener("resize", checkSize);
    return () => {
      window.removeEventListener("resize", checkSize);
    };
  }, []);
  return React.createElement("div", {
    className: `menu-item ${isSelected ? "menu-item-selected" : ""}`,
    onClick: () => {
      selectCallback(itemName);
    }
  }, React.createElement("img", {
    width: "40px",
    height: "40px",
    src: itemIcon,
    className: "menu-item-icon"
  }), hasLabel && React.createElement("label", {
    className: "menu-item-label"
  }, itemName));
};
var Menu = ({ itemsState, selectCallback }) => {
  return React.createElement("section", {
    className: "menu"
  }, React.createElement("div", {
    className: "menu-main"
  }, React.createElement("header", {
    className: "menu-header"
  }, React.createElement("img", {
    width: 50,
    height: 50,
    src: "data:image/png;base64,",
    className: "menu-logo"
  })), React.createElement("hr", null), React.createElement("section", {
    className: "menu-items"
  }, React.createElement(MenuItem, {
    itemIcon: options_settings_icon_default,
    itemName: "Settings",
    isSelected: itemsState.settings,
    selectCallback
  }), React.createElement(MenuItem, {
    itemIcon: help_icon_default,
    itemName: "Help",
    isSelected: itemsState.help,
    selectCallback
  }), React.createElement(MenuItem, {
    itemIcon: privacy_icon_default,
    itemName: "Privacy",
    isSelected: itemsState.privacy,
    selectCallback
  }), React.createElement(MenuItem, {
    itemIcon: customize_icon_default,
    itemName: "Customize",
    isSelected: itemsState.customize,
    selectCallback
  })), React.createElement("hr", null)), React.createElement("footer", {
    className: "menu-footer"
  }, React.createElement("img", {
    src: contact_icon_default,
    className: "menu-footer-item"
  }), React.createElement("img", {
    src: github_icon_default,
    className: "menu-footer-item"
  })));
};
var OptionsPage = () => {
  const [selectedItem, setSelectedItem] = React.useState({
    settings: true,
    help: false,
    privacy: false,
    customize: false
  });
  const onSelect = (itemName) => {
    setSelectedItem({
      settings: itemName === "Settings",
      help: itemName === "Help",
      privacy: itemName === "Privacy",
      customize: itemName === "Customize"
    });
  };
  return React.createElement("main", {
    className: "main-section"
  }, React.createElement(Menu, {
    itemsState: selectedItem,
    selectCallback: onSelect
  }), React.createElement("section", {
    className: "menu-item-content"
  }, selectedItem.customize && React.createElement(CustomizeSection, null), selectedItem.help && React.createElement(HelpSection, null), selectedItem.privacy && React.createElement(PrivacySection, null), selectedItem.settings && React.createElement(SettingsSection, null)));
};
var root = client.createRoot(document.getElementById("root"));
root.render(React.createElement(OptionsPage, null));
